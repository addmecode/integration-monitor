...
- [ ] **Create the `Test/` app project.** Add a `Test/` folder as a sibling of `integration-monitor/`, with its own `app.json`: `target: Test`, id range 50141–50180, same `runtime`/feature flags as the main app, and dependencies on the Integration Monitor app plus the test framework apps (`Tests-TestLibraries`, `Library Assert`, `Any`, `System Application Test`, `Test Runner`). Mirror the App's feature folder structure inside `Test/`.
- [ ] **Register the test folder with AL-Go.** Add the `Test/` folder to `.AL-Go/settings.json` under `testFolders` so the pipeline compiles and runs it. Confirm the main app stays in `appFolders`.
- [ ] **Add a smoke test.** Create one trivial `[Test]` procedure that asserts `true` and confirm the test app builds and the test runner executes it green. This validates the whole scaffold before any real test is written.
- [ ] **Add the `AMC Test Library` factory codeunit.** Provide reusable, deterministic setup helpers so tests stay DRY: `CreateMessageSetup(MessageType; Enabled; MaxAttempts; BaseRetryDelaySec)` returning a configured `AMC Int. Message Setup`; `CreateOutboxEntry(MessageType; Status)` and `CreateInboxEntry(...)`; `CreateAuthProfile(AuthType; WithSecret)`; `WriteBlobText(var Rec; FieldNo; Text)`. Use `Any` for random-but-valid values where the exact value does not matter.
- [ ] **Add a mock HTTP transport for processor send-path tests.** The processor resolves the transport from the `AMC Int. Transport Type` enum via the `AMC IHttpTransportHandler` interface. To inject a fake response without real HTTP, make `AMC Int. Transport Type` `Extensible = true` (production change — call it out in the commit), then add a test enum-extension value bound to a mock handler codeunit implementing `AMC IHttpTransportHandler`. The mock returns a caller-configured `HttpResponseMessage` (status code + body) so Phase 6 can drive 200/500 paths deterministically.
...
- [ ] **OnDelete is blocked while Sending.** Given an Outbox entry with `Status = Sending`, when deleted, then it errors with "Cannot delete record because the outbox entry is being sent."
- [ ] **OnDelete is blocked when a related Inbox entry is Processing.** Given an Outbox entry with a related Inbox entry (`Outbox Entry No.` matches) whose `Status = Processing`, when the Outbox entry is deleted, then it errors with "Cannot delete record because a related Inbox Entry is being processed."
...
- [ ] **OnInsert defaults timestamps.** Given a new Inbox entry with `Created At`/`Next Attempt At` = `0DT`, when inserted, then both default to ≈ now; pre-set values are left unchanged.
...
- [ ] **Failure increments attempt count and marks Failed.** Given an entry with `Attempt Count = N` and a Message Setup with `Max Attempts > N+1`, when the failure handler runs, then `Attempt Count = N+1`, `Last Attempt At` ≈ now, `Processed At = 0DT`, and `Status = Failed`.
- [ ] **Next Attempt At applies linear backoff under Max Attempts.** Given the resulting `Attempt Count < Max Attempts` and `Base Retry Delay (sec) = D`, when the handler runs, then `Next Attempt At = Last Attempt At + D*1000 ms`.
